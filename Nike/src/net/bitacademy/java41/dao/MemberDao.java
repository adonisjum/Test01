...
public class MemberDao {
	...
	public Member getMember(String email, String password) throws Exception {
		...
		try {
			...
			stmt = con.prepareStatement(
				"select EMAIL,MNAME,TEL,LEVEL,PWD from SPMS_MEMBS "
	...
	public int update(Member member) throws Exception {
		...
		try {
			...
			stmt = con.prepareStatement(
				"update SPMS_MEMBS set"
				+ " MNAME=?,TEL=?,BLOG=?,DET_ADDR=?,TAG=?,REG_DATE=now()"
				+ " where EMAIL=?");
	...
	public List<Member> listByProject(int no) throws Exception {
		...
		try {
			...
			String sql = "select t1.MNAME,t1.EMAIL,t1.TEL,t1.BLOG ";
			sql += "from SPMS_MEMBS t1, SPMS_PRJMEMB t2, SPMS_PRJS t3";
			sql += " where t2.EMAIL=t1.EMAIL and t2.PNO=t3.PNO and t2.PNO=?;";
